...
export default class Yunzai extends EventEmitter {
  ...
  serverAuth(req) {
    ...
    if (cfg.server.auth) for (const i in cfg.server.auth) {
      if (req.query[i] === cfg.server.auth[i] || req.headers[i.toLowerCase()] === cfg.server.auth[i])
        continue
      req.res?.sendStatus(401)
      this.makeLog("error", ["HTTP", req.method, "请求", i, "鉴权失败", req.headers, req.query], `${req.sid} <≠ ${req.rid}`, true)
      return false
    }
    req.next?.()
  }
  ...
  wsConnect(req, socket, head) {
    ...
    if (!(path in this.wsf)) {
      this.makeLog("error", ["WebSocket 处理器", path, "不存在"], `${req.sid} <≠> ${req.rid}`, true)
      socket.write("HTTP/1.1 404 Not Found\r\n\r\n")
      return socket.destroy()
    }

    this.wss.handleUpgrade(req, socket, head, conn => {
      this.makeLog("mark", ["建立连接", req.headers], `${req.sid} <=> ${req.rid}`, true)
      conn.on("error", (...args) => this.makeLog("error", args, `${req.sid} <=> ${req.rid}`, true))
      conn.on("close", () => this.makeLog("mark", "断开连接", `${req.sid} <≠> ${req.rid}`, true))
      conn.on("message", msg => this.makeLog("debug", ["消息", this.String(msg)], `${req.sid} <= ${req.rid}`, true))
      conn.sendMsg = msg => {
        if (!Buffer.isBuffer(msg)) msg = this.String(msg)
        this.makeLog("debug", ["消息", msg], `${req.sid} => ${req.rid}`, true)
  ...
  async serverEADDRINUSE(err, https) {
    this.makeLog("error", ["监听端口", https ? cfg.server.https.port : cfg.server.port, "错误", err], "Server")
    if (https) return
    try {
  ...
  async serverLoad(https) {
    ...
    try {
      await this.promiseEvent(this[server], "listening", https && "error")
    } catch (err) {
      return
    }
    const { address, port } = this[server].address()
    this.makeLog("mark", [`启动 HTTP${https?"S":""} 服务器`, logger.green(`http${https?"s":""}://[${address}]:${port}`)], "Server")
  ...
  async httpsLoad() {
    try {
      ...
      this.makeLog("error", ["创建 HTTPS 服务器错误", err], "Server")
    }
  }
  ...
  async run() {
    ...
    this.makeLog("info", `连接地址：${logger.blue(`${this.url.replace(/^http/, "ws")}/`)}${logger.cyan(`[${Object.keys(this.wsf)}]`)}`, "WebSocket")
  ...
  fileSend(req) {
    ...
    this.makeLog("mark", `发送文件：${file.name}(${file.url} ${(file.buffer.length/1024).toFixed(2)}KB)`, `${req.sid} => ${req.rid}`, true)
  ...
  sendFriendMsg(bot_id, user_id, ...args) { try {
    ...
    this.makeLog("error", ["发送好友消息错误", args, err], `${bot_id} => ${user_id}`, true)
  }}
  ...
  sendGroupMsg(bot_id, group_id, ...args) { try {
    ...
    this.makeLog("error", ["发送群消息错误", args, err], `${bot_id} => ${group_id}`, true)
  }}
